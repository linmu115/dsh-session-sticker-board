...
type Props = { sessionId: string; local: Parameters<typeof migrateLegacyStickers>[1]; bridge: Parameters<typeof migrateLegacyStickers>[2]; onMigrated(): Promise<void> };
export function OrdinaryStickerTools(props: Props) {
  ...
  const migrate = async (choice?: 'local' | 'vault') => { const count = await migrateLegacyStickers(props.sessionId, props.local, props.bridge, choice); await props.onMigrated(); setMigrationConflict(false); setNotice(`已迁移 ${count} 张普通贴纸。`); };
  if (!open) return null;
