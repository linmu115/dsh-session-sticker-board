...
try{
  ...
  const note = await vault.dispatch('note-register', {notePath:'合成笔记.md'}, 'synthetic-instance');
  const selectedText = '🧠'.repeat(260) + '尾部不应显示';
  await knowledge('write', {namespace:'stickers',objectId:'note-selection-sticker',expectedRevision:0,title:'笔记选段讨论',body:{kind:'session',logicalSessionId:'logical-target',note:{...note,blockId:'source-selection'},noteSelection:{selectedText,selectedTextHash:'sha256:'+createHash('sha256').update(selectedText).digest('hex'),occurrence:0}}});
  await dialog.getByRole('button',{name:'关闭',exact:true}).click();await header().click();
  const noteCard = dialog.locator('.dsh-knowledge-card').filter({has:page.getByRole('button',{name:'笔记选段讨论',exact:true})});
  await noteCard.getByText('来源笔记：合成笔记.md',{exact:true}).waitFor();
  assert.equal(await noteCard.locator('blockquote').textContent(), '🧠'.repeat(240)+'…');
  await noteCard.getByText('仅建立关联，不自动送入模型',{exact:true}).waitFor();
  await Promise.all([page.waitForResponse(response => response.url() === bridge.origin + '/v1/knowledge/note-open'), noteCard.getByRole('button',{name:'打开来源笔记',exact:true}).click()]);
  assert.deepEqual(openedNotes.at(-1), {path:'合成笔记.md',blockId:'source-selection'});
  assert.equal(await dialog.isVisible(),true);assert.equal(await page.evaluate(()=>fixture.refs.length),1);
  assert.equal(await page.evaluate(()=>fixture.draft),'用户原有草稿');
  await noteCard.getByRole('button',{name:'笔记选段讨论',exact:true}).click();await dialog.waitFor({state:'hidden'});
  assert.equal(await page.evaluate(()=>fixture.opened.at(-1)),'native-target');await header().click();
  checks.push('note-selection sticker shows a bounded Unicode excerpt; authenticated note-open preserves block identity, primary opens full session, no model reference or draft mutation');
