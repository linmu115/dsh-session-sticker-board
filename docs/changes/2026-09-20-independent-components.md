...
对应候选版本：0.7.4-rc2.6。依据用户已确认的 2026-09-20 架构文档与本次「按文档对各个插件进行架构升级」指令。

## 已实现

必需 Core 和 Bridge；侧栏插件改为可选；持久修改消费 Core 公共写入许可，已受管实例失联停写。

技术入口：[实现](../../src/remote/service.ts)。本轮只有源码和候选产物，尚未安装到实际实例；不以单元测试替代真实数据验收。
