...
describe("sticker-board 0.6 package boundary", () => {
  it("keeps a standalone single-node patch and declares all ordinary Sticker feature peers", async () => {
    ...
    expect(pkg.devDependencies['dsh-obsidian-bridge']).toBe('file:vendor/dsh-obsidian-bridge-0.4.1-rc2.11.tgz');
  });
  ...
  it("declares version-open shared Core and host peers", async () => {
    ...
    expect(packageJson.version).toBe("0.7.4-rc2.12");
    expect(packageJson.peerDependencies["@deepseek-ai/dsh-typert-protocol"]).toBe("^0.1.5-rc.2");
    expect(packageJson.peerDependencies["dsh-annotation-core"]).toBe("0.3.12-rc2.29");
    expect(packageJson.peerDependencies["dsh-obsidian-bridge"]).toBe("0.4.1-rc2.11");
