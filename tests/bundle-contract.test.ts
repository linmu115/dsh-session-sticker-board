...
describe("sticker-board 0.6 package boundary", () => {
  it("declares version-open shared Core and host peers", async () => {
    ...
    expect(packageJson.version).toBe("0.7.3-rc2.1");
    expect(packageJson.peerDependencies["@deepseek-ai/dsh-typert-protocol"]).toBe("^0.1.5-rc.2");
    expect(packageJson.peerDependencies["dsh-annotation-core"]).toContain("0.3.11-rc2.1");
    expect(packageJson.peerDependencies["dsh-obsidian-bridge-lifecycle"]).toBe("^0.3.2 || 0.3.3-rc2.1");
