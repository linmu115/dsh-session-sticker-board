...
import { selectedTextHash, documentHash } from '../src/protocol.ts';
...
import type { StickerBridgeChannel } from '../src/client/bridge-channel.ts';
vi.mock('../src/client/knowledge.ts',()=>({knowledgeRequest:vi.fn()}));
let root:ReturnType<typeof createRoot>,host:HTMLDivElement;
const referenceSource = {sourceType:'obsidian-note' as const,selectedText:'quote',locator:{vaultId:'vault',notePath:'Folder/Note.md',blockId:'block',occurrence:0,selectedTextHash:selectedTextHash('quote')},snapshot:{markdown:'quote',documentHash:documentHash('quote'),capturedAt:1,freshness:'captured' as const}};
...
it('hands a validated relation to the shared Bridge without owning Core writes',async()=>{const f=await fixture();await click('Note');await click('引用到本轮');expect(f.bridge.handoffReference).toHaveBeenCalledWith(expect.objectContaining({sessionId:'native',operationId:expect.any(String)}));expect(f.core.addReference).not.toHaveBeenCalled();expect(f.bridge.knowledge.mock.calls.map(c=>c[0])).toEqual(['link-reference-prepare','link-reference-commit']);expect(f.bridge.knowledge).toHaveBeenLastCalledWith('link-reference-commit',expect.objectContaining({setId:'set',objectId:'link',nativeSessionId:'native',logicalSessionId:'logical',profileId:'web'}));expect(host.textContent).toContain('发送消息后参与回答');});
it('refuses to add after switching sessions during preparation',async()=>{const f=await fixture();f.bridge.knowledge.mockImplementationOnce(async()=>{f.ctx.sessions.list.getSnapshot=()=>({current:'other'});return{referenceId:'ref',source:referenceSource};});await click('Note');await click('引用到本轮');expect(f.core.addReference).not.toHaveBeenCalled();expect(host.textContent).toContain('会话已切换');});
it('surfaces handoff failures and leaves compensation to Bridge and Core',async()=>{const f=await fixture();f.bridge.handoffReference.mockRejectedValueOnce(new Error('无法保存引用'));await click('Note');await click('引用到本轮');expect(f.core.discardPendingOperation).not.toHaveBeenCalled();expect(host.textContent).toContain('无法保存引用');expect(host.textContent).not.toContain('已加入本轮引用');});

it('rejects a removed relation before handing off',async()=>{const f=await fixture();vi.mocked(knowledgeRequest).mockImplementation(async(op:any)=>op==='resolve'?{nativeSessionId:'native',logicalSessionId:'logical'}:{...object,deleted:true});await click('Note');await click('引用到本轮');expect(f.bridge.handoffReference).not.toHaveBeenCalled();expect(host.textContent).toContain('关联已解除');});
