...
“复制笔记链接”同时生成贴纸伴生块的原生 WikiLink 和 `obsidian://deepharness?...`
可执行链接。WikiLink 由 Obsidian 原生反向链接索引维护；协议链接只负责打开或聚焦
Obsidian 内的 DSH Web Viewer，再定位到原消息。

贴纸详情下方显示通过原生 WikiLink 引用当前贴纸的 Obsidian 笔记。点击反向链接时优先
定位到稳定块 ID，没有块 ID 时回退到链接所在行；列表可手动刷新。
